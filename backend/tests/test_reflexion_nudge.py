...
大部分用例先安排一轮成功的verify_number调用满足自我核查底线——不然模型第一次
end_turn时会先被自我核查兜底拦下来（那是另一套独立的拦截，见
test_self_verification_nudge.py），会干扰"这里到底测的是reflexion"这件事。
"""
...
def _verify_number_turn() -> LLMResponse:
    return LLMResponse(
...
def test_reflexion_injects_critique_and_forces_another_turn_when_check_returns_text():
    ...
    assert result.final_report == "<conclusion>结论（已整改）</conclusion>"
    # 批评文字确实被塞进了第3轮的对话历史里
...
def test_reflexion_only_fires_once_even_if_check_keeps_returning_critique():
    ...
    responses = [
        _verify_number_turn(),
        LLMResponse(stop_reason="end_turn", text="<conclusion>结论</conclusion>", tool_calls=[], raw=None),
        LLMResponse(stop_reason="end_turn", text="<conclusion>结论（仍不完美）</conclusion>", tool_calls=[], raw=None),
    ]
    fake = FakeLLMClient(responses)
    check_calls = 0
...
def test_self_verification_nudge_takes_priority_over_reflexion_in_the_same_round():
    ...
    （核实过数字）解决掉，reflexion_check在这一轮根本不会被调用。"""
    responses = [
        LLMResponse(stop_reason="end_turn", text="<conclusion>结论</conclusion>", tool_calls=[], raw=None),
        LLMResponse(stop_reason="end_turn", text="<conclusion>结论（已核查）</conclusion>", tool_calls=[], raw=None),
    ]
    fake = FakeLLMClient(responses)
    ...
    # 第一轮触发的是自我核查兜底，不是reflexion——reflexion_check直到第二轮
    # （自我核查过关之后）才会第一次被调用
    reflexion_check.assert_called_once()
    assert result.self_verification_triggered is False  # 两轮都没真的调用verify_number
    assert result.reflexion_triggered is False


def test_reflexion_triggered_reflects_state_on_max_turns_exceeded():
    """max_turns=2：第0轮先核实一个数字（满足自我核查底线），第1轮end_turn时
    自我核查已经过关、轮到reflexion_check介入并返回批评——插入提示后continue，
    但预算已经耗尽（range(2)只有0/1两轮），没有第3轮真正把修正版写出来，
    落到max_turns_exceeded分支。reflexion_triggered应该如实反映"确实触发过"，
    不因为最终没能在预算内收尾就被抹成False。"""
    responses = [
        _verify_number_turn(),
        LLMResponse(stop_reason="end_turn", text="<conclusion>结论</conclusion>", tool_calls=[], raw=None),
    ]
